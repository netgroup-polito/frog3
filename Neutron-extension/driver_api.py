...
#class added by Matteo
@six.add_metaclass(ABCMeta)
class FlowruleContext(object):
    """Context passed to MechanismDrivers for changes to flowrule resources.

    A flowrule instance wraps a flowrule resource. It provides
    helper methods for accessing other relevant information. Results
    from expensive operations are cached so that other
    MechanismDrivers can freely access the same information.
    """
    ...
    @abstractproperty
    def current(self):
        """Return the current state of the flowrule.

        Return the current state of the flowrule, as defined by
        NeutronPluginBaseV2.create_flowrule and all extensions in the
        ml2 plugin.
        """
        pass
    ...
    @abstractproperty
    def original(self):
        """Return the original state of the flowrule.

        Return the original state of the flowrule, prior to a call to
        update_flowrule. Method is only valid within calls to
        update_flowrule_precommit and update_flowrule_postcommit.
        """
        pass
