...
class FlowruleNotFound(NotFound):
    message = _("Flowrules %(flowrule_id)s could not be found")
    
class FlowruleIdDuplicated(Conflict):
    message = _("Flowrule with id %(flowrule_id)s already exists.")

class NotYetImplemented(NotFound):
    message = _("this feature is not yet implemented")
    
#MD exceptions
class PortTranslationNotPossible(NotFound):
    message = _("Port %(port_id)s could not be found in any vswitch")

class UnknownAction(NotFound):
    message = _("Action %(action)s could not be processed, supported actions are OUTPUT=port and DROP")

class NonLinkablePorts(NotFound):
    message = _("ports %(id1)s and %(id2)s are not linkable")

class NoFreeVlan(NotFound):
    message = _("no free vlans id available")

class PortNotActive(NotAuthorized):
    message = _("the port %(port_id) is not active ad therefore the flow cannot be istanciated")