...
class Ml2Plugin(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    _supported_extension_aliases = ["provider", "external-net", "binding",
                                    ...
                                    "extra_dhcp_opt","flowrule"]
#added by Matteo ', "flowrule"'
    ...
    def delete_port(self, context, id, l3_port_check=True):
        ...
        '''
        modified by Matteo to remove the lock on the DB
        otherwise, since the delete_flowrule is called it cannot access the DB
        tests look ok
        
        with contextlib.nested(lockutils.lock('db-access'),
                               session.begin(subtransactions=True)):
        '''
        with contextlib.nested(session.begin(subtransactions=True)):
            try:
                port_db = (session.query(models_v2.Port).
                           enable_eagerloads(False).
                           filter_by(id=id).with_lockmode('update').one())
            ...
            #Matteo: deleting associated flowrules (leap 1->single input, 0:1 output)
            
            flowrules_that_come_out = (session.query(models_v2.Flowrule).
                           enable_eagerloads(False).
                           filter_by(ingressPort=id))
            flowrules_that_come_in = (session.query(models_v2.Flowrule).
                           enable_eagerloads(False).
                           filter_by(actions="OUTPUT="+id))
                           
            for flowrule in flowrules_that_come_out:
                self.delete_flowrule(context, flowrule['id'])
            for flowrule in flowrules_that_come_in:
                self.delete_flowrule(context, flowrule['id'])
...
#from here to the bottom, the code was added by Matteo
    ...
    def create_flowrule(self, context, flowrule):
        '''
        called by a POST to /flowrules or /flowrules.json
        the received json is in flowrule in the form of dictionary
        in here 3 methods are called:
        
        HOW THINGS SHOULD WORK
        create_flowrule_precommit -> mechanism driver method (it should store in the "flowmods" DB table the flowmods to be instanciated)
        create_flowrule -> calls create_flowrule in db_base_plugin_v2.py (basically stores the flowrule in the "flowrules" table plus some checks)
        create_flowrule_postcommit -> mechanism driver method to istanciate flowmods in ODL 
        
        HOW THINGS REALLY WORK -- old review, NOW IT WORKS AS IT IS SUPPOSED TO
        create_flowrule_precommit -> performs only some basic checks
        create_flowrule -> calls create_flowrule in db_base_plugin_v2.py (basically stores the flowrule in the "flowrules" table plus some checks)
        create_flowrule_postcommit -> flow path calculation + vlan incapsulation + flowmods creation + flowmods POST to ODL
        '''
        LOG.debug(_("create_flowrule"))
        LOG.debug(_("flowrule to be created: "+str(flowrule)))
        result = None
        session = context.session
        with session.begin(subtransactions=True):
            LOG.debug(_("PRE MD call"))
            #creating the context
            mech_context = driver_context.FlowruleContext(self,
                                                         context,
                                                         flowrule)
            #in theory, i should send rules to opendaylight already in the precommit
            #leaving them in the non active mode. 
            #then in the post commit i should trigger them to the active mode
            #since this is a trial version, i "trust" the components so all calls
            #are done in the postcommit method according to the phylosophy
            #of the mechanism driver principles
            
            #here i have to "translate" the rule, making sure I have a specific  port in the input_port field
            #in the case of the input port being null, it means that i should apply the rule on all ports of this tenant
            
                        
            self.mechanism_manager.create_flowrule_precommit(mech_context)
            result = super(Ml2Plugin, self).create_flowrule(context, flowrule)
            
            #TBD move out of the transaction raising a proper error
            self.mechanism_manager.create_flowrule_postcommit(mech_context)
        return result

    def get_flowrule(self, context, id, fields=None):
        ''' returns the flowrule with the given id'''
        LOG.debug(_("get_flowrule with id"))
        LOG.debug(_(id))
        session = context.session
        with session.begin(subtransactions=True):
            result = super(Ml2Plugin, self).get_flowrule(context, id, None)
            #self._extend_network_dict_provider(context, result)
        return self._fields(result, fields)

    def get_flowrules(self, context, filters=None, fields=None,
                     sorts=None, limit=None, marker=None, page_reverse=False):
        '''returns all the flowrules for this user'''
        ...
        LOG.info(_("get_flowrules"))
        session = context.session
        with session.begin(subtransactions=True):
            flowrules = super(Ml2Plugin,
                         self).get_flowrules(context, filters, None, sorts,
                                            limit, marker, page_reverse)


        return [self._fields(flowrule, fields) for flowrule in flowrules]
    ...
    def update_flowrule(self, context, id, flowrule):
        # The id is the unique identifier to your entry, flow is a
        # dictionary with values that needs to be updated with.
        LOG.debug(_("update_flowrule with id "+str(id)))
        raise exc.NotYetImplemented
