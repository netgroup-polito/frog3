...
from Common.SQL.session import get_profile
...
class NF_FG(object):
    ...
    def __init__(self, nf_fg):
        ...
        for vnf in nf_fg['profile']['VNFs']:
            ...
            self.vnf = VNF(vnf['name'], vnf['vnf_descriptor'], vnf['id'], vnf['ports'], manifest = manifest)
    ...
    def createEndpoint(self, name, edge = False):
    ...
    def addControlSwitch(self, endpoint_name = None):
        if self.control_switch_label is False:
            ...
            port.setFlowRuleToEndpoint(user_control_egress)
            port.setFlowRuleFromEndpoint(self.control_switch, user_control_egress)
    ...
    def connectVNFToEndpoint(self, vnf, port, endpoint):
        port.setFlowRuleToEndpoint(endpoint)
        port.setFlowRuleFromEndpoint(endpoint)
          
    def connectTwoVNFs(self, vnfA, portA, vnfB, portB):
        portB.setFlowRuleToVNF(vnfA, portA)
        portA.setFlowRuleToVNF(vnfB, portB)
    ...
    def createEndpointSwitch(self, endpoint):
        ...
        port.setFlowRuleToEndpoint(endpoint)
        port.setFlowRuleFromEndpoint(endpoint_switch, endpoint)
        return endpoint_switch
    ...
    def addEdgeEndpoint(self, endpoint_switch): 
        ...
        port = self.addPortToSwitch(endpoint_switch)
        port.setFlowRuleToEndpoint(edge_endpoint)
        port.setFlowRuleFromEndpoint(endpoint_switch, edge_endpoint)
...
class Endpoint(object):
    ...
    def connectToExternalNF_FGWithoutEdgeEndpoint(self, nf_fg, ext_nf_fg, endpoint_name):
        '''
        Returns the edge endpoint to connect
        '''
        #TODO: should be moved to /ServiceLayerApplication/common
        
        # Get available endpoint of remote graph
        remote_edge_endpoint = get_first_available_endpoint_by_name(ext_nf_fg._id, endpoint_name)
        if remote_edge_endpoint is None:
            raise connectionsError("No remote 'edge_endpoint' found")
        ...
        for port in ports:
            for flowrule in port.list_outgoing_label:   
                if flowrule.action.endpoint is not None and getEndpointID(flowrule.action.endpoint) == self.id:
                    if remote_edge_endpoint.Type == "interface":
                        nf_fg.getEndpointMap()[getEndpointID(flowrule.action.endpoint)].type = 'physical'
                        nf_fg.getEndpointMap()[getEndpointID(flowrule.action.endpoint)].interface = remote_edge_endpoint.Endpoint_ID
        for port in ports:
            for flowrule in port.list_ingoing_label:
                if 'ingress_endpoint' in flowrule.flowspec and flowrule.flowspec['ingress_endpoint'] == self.id:
                    if remote_edge_endpoint.Type == "interface":
                        nf_fg.getEndpointMap()[flowrule.flowspec['ingress_endpoint']].interface = remote_edge_endpoint.Endpoint_ID
                        nf_fg.getEndpointMap()[flowrule.flowspec['ingress_endpoint']].type = 'physical'
        # TODO: we shouldn't change the endpoint id
        # TODO: we need also to store the nf-fg id of remote endpoint
        self.remote_graph_name = ext_nf_fg.name
        self.remote_id = remote_edge_endpoint.Endpoint_ID
        self.remote_graph = ext_nf_fg._id
        self.connection = True        
        updateEndpointConnection(ext_nf_fg._id, remote_edge_endpoint.Endpoint_ID,
                                  nf_fg._id, self._id)
    
    def connectToExternalNF_FG(self, nf_fg, ext_nf_fg, endpoint_name):
        '''
        Returns the edge endpoint to connect
        '''
        remote_edge_endpoint = nf_fg.findEdgeEndpoint(ext_nf_fg, endpoint_name)
        if remote_edge_endpoint is None:
            raise connectionsError("No remote 'edge_endpoint' found")
        #self.connections.append(Connection(ext_nf_fg, remote_edge_endpoint))
        # Change id
        for port in self.endpoint_switch.listPort:
            for flowrule in port.list_ingoing_label:
                if 'ingress_endpoint' in flowrule.flowspec and flowrule.flowspec['ingress_endpoint'] == self.id:
                    flowrule.flowspec['ingress_endpoint'] = ext_nf_fg._id +":"+ remote_edge_endpoint._id
            for flowrule in port.list_outgoing_label:   
                if flowrule.action.endpoint is not None and getEndpointID(flowrule.action.endpoint) == self.id:
                    flowrule.action.endpoint['id'] = ext_nf_fg._id +":"+ remote_edge_endpoint._id
        self._id = ext_nf_fg._id +":"+ remote_edge_endpoint._id
        self.connection = True
        remote_edge_endpoint.connections.append(Connection(nf_fg._id, self._id))
        return remote_edge_endpoint         
          
class VNF(object):
    
    def __init__(self, VNFname, template, vnf_id = None, ports = None, manifest = None):
        ...
        if ports is not None:
            for port in ports:
                if 'ingoing_label' in port:
                    self.port = Port(port['id'], outgoing_label = port['outgoing_label'], ingoing_label = port['ingoing_label'], vnf_id = self.id)
                else:
                    self.port = Port(port['id'], outgoing_label = port['outgoing_label'], vnf_id = self.id)
...
class Port(object):
    def __init__(self, port_id, outgoing_label = None, ingoing_label= None, vnf_id = None):
        self._id = port_id
        self.vnf_id = vnf_id
        self.outgoing_label = {}
    ...
    def setFlowRuleToEndpoint(self, endpoint):
        matches = []
        matches.append(Match(priority = DEFAULT_PRIORITY, match_id = uuid.uuid4().hex))
        endpoint_pointer  = {}
        endpoint_pointer['port'] = endpoint.id
    ...
    def setFlowRuleFromEndpoint(self, vnf, endpoint):
        matches = []
        matches.append(Match(priority = DEFAULT_PRIORITY, match_id = uuid.uuid4().hex))
        vnf_pointer  = {}
        vnf_pointer['id'] = vnf.id
        vnf_pointer['port'] = self.id
        action = Action("output", vnf = vnf_pointer)
        flowrule = Flowrule(action, matches = matches, ingress_endpoint = endpoint.id)
        self.list_ingoing_label.append(flowrule)
        self.flowrules.append(flowrule)
        
    def setFlowRuleToVNF(self, vnf, port):
        matches = []
        matches.append(Match(priority = DEFAULT_PRIORITY, match_id = uuid.uuid4().hex))
        vnf_pointer  = {}
        vnf_pointer['id'] = vnf.id
        vnf_pointer['port'] = port.id
