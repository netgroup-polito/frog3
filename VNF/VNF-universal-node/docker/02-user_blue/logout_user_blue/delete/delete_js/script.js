...
{
	var keystone_ip = "192.168.1.113";
	var keystone_port = "35357";
	var orchestrator_ip = "192.168.1.113";
	var orchestrator_port = "8000";
	var token = null;
	
		
	token = getTokenFromKeystone(keystone_ip, keystone_port);
	sendDeleteToGlobalOrch(orchestrator_ip, orchestrator_port, token);
    ...
    xmlhttp.setRequestHeader("Content-Type", "application/xml; charset=utf-8");
	xmlhttp.open("DELETE","orchestrator?username="+document.forms[0].username.value+"&password="+document.forms[0].password.value,false);
	*/

}

function getTokenFromKeystone(addr, port)
{
	var method = "POST";
	var token = null;
	
	var xmlhttp = new XMLHttpRequest();
	var body = '{"auth": {"tenantName": "blue", "passwordCredentials": {"username": "blue", "password": "stack"}}}'
	var header1 = "Content-Type: application/json"
	var header2 = "Accept: application/json"
    //var remove_url = "http://127.0.0.1/delete/proxy.php?headers=false&header_send[]="+header1+"&header_send[]="+header2+"&method="+method+"&url=http://"+addr+":"+port+"/v2.0/tokens&data="+body;
	var remove_url = "http://"+ip+"/proxy.php?headers=false&header_send[]="+header1+"&header_send[]="+header2+"&method="+method+"&url=http://"+addr+":"+port+"/v2.0/tokens&data="+body;
...
function sendDeleteToGlobalOrch(addr, port, token)
{

    var button = document.getElementById("button_logout");
	button.style.display = "none";
	var logged = document.getElementById("logged");
	logged.style.display = "none";
	var logged_out = document.getElementById("logout");
    ...
    var remove_url = "http://"+ip+"/proxy.php?headers=true&header_send[]="+header+"&method="+method+"&url=http://"+addr+":"+port+"/orchestrator";
