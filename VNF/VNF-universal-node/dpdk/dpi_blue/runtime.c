...
*/
#define CNN	"telecomitalia.com"
//XXX: add here other words

/**
*	Variables related to the forbidden words in HTTP requests
*/
pcre *re_cnn;				//Compiled regular expression (for libpcre)
pcre_extra *re_extra_cnn;	//Data to speed up matching (libpcre-specific)
...
int do_nf(void *useless)
{
	...
	if(!initializeRegEx(&re_cnn, re_extra_cnn,CNN))
		return 0;
...
int drop(unsigned char *packet, unsigned int len)
{
	...
	int rc = pcre_exec (re_cnn, re_extra_cnn, (const char*) http, len, 0, 0, ovector, OVECTSIZE);
