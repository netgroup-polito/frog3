...
*/
#define BBC	"telekom.com"
//XXX: add here other words

/**
*	Variables related to the forbidden words in HTTP requests
*/
pcre *re_bbc;				//Compiled regular expression (for libpcre)
pcre_extra *re_extra_bbc;	//Data to speed up matching (libpcre-specific)
...
int drop(unsigned char *packet, unsigned int len)
{
	...
	int rc = pcre_exec (re_bbc, re_extra_bbc, (const char*) http, len, 0, 0, ovector, OVECTSIZE);
