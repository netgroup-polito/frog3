...
#from here to the bottom the code was added by Matteo
    '''functions called by plugin.py'''
    def create_flowrule(self, context, flowrule):
        '''
        stores the flowrule in the DB (.add function)
        '''
        LOG.debug(_("Matteo_create_flowrule_db_base_plugin_v2_begin"))
        f = flowrule['flowrule']
        tenant_id = self._get_tenant_id_for_create(context, f)
        flowrule_id = f['id']
        with context.session.begin(subtransactions=True):
            #since new id type is defined, i need to be sure that is is unique
            id_already_exists = True
            try:
                flowrule = self._get_flowrule(context, flowrule_id)
            except exc.NoResultFound: 
                id_already_exists=False
            if id_already_exists:
                #raise n_exc.DeviceIDNotOwnedByTenant(device_id=flowrule_id)
                raise n_exc.FlowruleIdDuplicated(flowrule_id=flowrule_id)
            #now creating a new flowrule if the id is valid
            args = {
                        'id': flowrule_id,
                        'tenant_id': tenant_id,
            ...
            LOG.debug(_(args))
            flowrule = models_v2.Flowrule(**args)
            context.session.add(flowrule)
        return self._make_flowrule_dict(flowrule, process_extensions=False)
    
    def _make_flowrule_dict(self, flowrule, fields=None, process_extensions=True):
        '''
        maps the resource
        '''
        res = {
                'id': flowrule['id'],
                'tenant_id':flowrule['tenant_id'],
                'hardTimeout': flowrule['hardTimeout'],
                'priority': flowrule['priority'],
                'ingressPort': flowrule['ingressPort'],
                ...
                'actions': flowrule['actions']
               }
        # Call auxiliary extend functions, if any
        if process_extensions:
            self._apply_dict_extend_functions(
                attributes.FLOWRULES, res, flowrule)
        return self._fields(res, fields)
    
    def get_flowrule(self, context, id, fields=None):
        LOG.debug(_("Matteo_get_flowrule_db_base_plugin_v2_begin"))
        flowrule = self._get_flowrule(context, id)
        LOG.debug(_("after _GET_FLOWRULE"))
        a = self._make_flowrule_dict(flowrule, fields)
        LOG.debug(_("Matteo_get_flowrule_db_base_plugin_v2_end"))
        return a
    
    def _get_flowrule(self, context, id):
        LOG.debug(_("Matteo__get_flowrule_db_base_plugin_v2_begin"))
        try:
            flowrule = self._get_by_id(context, models_v2.Flowrule, id)
        except n_exc.FlowruleNotFound:
            raise n_exc.FlowruleNotFound(flowrule_id=id)
        LOG.debug(_("Matteo__get_flow_db_base_plugin_v2_end"))
        return flowrule

    def get_flowrules(self, context, filters=None, fields=None,
                     sorts=None, limit=None, marker=None,
                     page_reverse=False):
        LOG.debug(_("Matteo_get_flowrules_db_base_plugin_v2_begin"))
        marker_obj = self._get_marker_obj(context, 'flowrule', limit, marker)
        items = self._get_collection(context, models_v2.Flowrule,
                                    self._make_flowrule_dict,
                                    filters=filters, fields=fields,
                                    sorts=sorts,
                                    limit=limit,
                                    marker_obj=marker_obj,
                                    page_reverse=page_reverse)
        LOG.debug(_("Matteo_get_flowrules_db_base_plugin_v2_end"))
        return items

    def delete_flowrule(self, context, id):
        with context.session.begin(subtransactions=True):
            self._delete_flowrule(context, id)
            
    def _delete_flowrule(self, context, id):
        query = (context.session.query(models_v2.Flowrule).
                 enable_eagerloads(False).filter_by(id=id))
        if not context.is_admin:
            query = query.filter_by(tenant_id=context.tenant_id)
        query.delete()
    ...
    """
    def get_flowrules_count(self, context, filters=None):
        LOG.debug(_("Matteo_get_flowrules_count_db_base_plugin_v2_begin"))
        a = self._get_flowrules_query(context, filters).count()
        LOG.debug(_("Matteo_get_flowrules_count_db_base_plugin_v2_end"))
        return a
    def update_flow(self, context, id, flow):
        LOG.debug(_("Matteo_update_flow_db_base_plugin_v2_begin"))
        LOG.debug(_("**********Matteo_update_flow_db_base_plugin_v2_NOT_YET_IMPLEMENTED*******"))
        LOG.debug(_("Matteo_update_flow_db_base_plugin_v2_end"))
        raise TBD
    
    def delete_flows_by_port_id(self, context, port_id=None):
        LOG.debug(_("Matteo_delete_flows_by_port_id_db_base_plugin_v2_begin"))
        query = (context.session.query(models_v2.Flow.id)
                 .enable_eagerloads(False)
                 .filter(models_v2.Flow.input_port == port_id))
        flow_ids = [f[0] for f in query]
        for flow_id in flow_ids:
            try:
                self.delete_flow(context, flow_id)
            except n_exc.FlowNotFound:
                # Don't raise if something else concurrently deleted the port
                LOG.debug(_("Ignoring FlowNotFound when deleting flow '%s'. "
                            "The flow doesn't exist."),
                            flow_id)
        LOG.debug(_("Matteo_delete_flows_by_port_id_db_base_plugin_v2_end"))
    ...
    '''now defining functions for the mechanism driver'''
    def store_port_switch_associaton(self, context, sw_id, odl_port_id, odl_port_name, ML2_port_id=None):
        '''
        this stores a tuple (switch_id, port# on the switch, name given by odl, ML2_port (maybe useful in the future))
        '''
        args = {
            'sw_id':sw_id,
            'odl_port_id': odl_port_id,
            #'odl_port_name': odl_port_name[3:],#TBD when introducing the "like" clausole, this and the get_switch_and_port_by_ML2_port_id method should be changed
            'odl_port_name':odl_port_name,
            'ML2_port_id': ML2_port_id
        }
        LOG.debug(_(args))
        line = models_v2.Switches_ports_association(**args)
        context.session.add(line)
       
    def update_ports_switch(self, context, sw_id, port_list):#sw_id is a string, port_list is a dict in the form [{'odl_id','name'}]
        '''
        stores in the db all the ports in the ODL switches
        should get a list of ports, checks the into the db, adds the new ones and delete the missing ones
        for the moment, deletes all ports and recreates them
        '''
        query = (context.session.query(models_v2.Switches_ports_association).
                 enable_eagerloads(False).filter_by(sw_id=sw_id))
        query.delete()
        for p in port_list:
            LOG.debug(_("storing port "+p['odl_id']+" aka "+ p['name']+" for sw="+sw_id))
            self.store_port_switch_associaton(context, sw_id, p['odl_id'], p['name'])
        
        return 
    
    def get_switch_and_port_by_ML2_port_id(self, context, ML2_port_id):#empiric method, only works for tap ports (VM ports)
        '''
        gets a port and the corresponding switch from the database starting from the ML2_port_id
        empiric method, only works for tap ports (VM ports)
        commented below there is a way to do it but neutron does NOT support the sqlalchemy "like" function YET (feature in code rewies)
        '''
        
        #ports with the prefix "INGRESS_" are managed in a different way since neutron
        #does NOT have them in the db BUT they will be retrieved from ODL
        if ML2_port_id[:8] == 'INGRESS_':
            switch_datapath_id = ':'.join([ML2_port_id[i:i+2] for i in range(8,len(ML2_port_id[8:24])+8,2)])
            LOG.debug(_("port "+ML2_port_id[25:]+" in bridge "+switch_datapath_id+" unknown to neutron-->going directly to ODL"))
            query = (context.session.query(models_v2.Switches_ports_association).
                     enable_eagerloads(False).filter_by(odl_port_name=ML2_port_id[25:]).filter_by(sw_id=switch_datapath_id))
        else:
            #TBD this should become a "like" filter when this possibility is introduced to neutron
            empiric_odl_port_name_for_VMs = 'tap'+ML2_port_id[:11]
            empiric_odl_port_name_for_routers = 'qr-'+ML2_port_id[:11]
            LOG.debug(_("name translation "+ML2_port_id+"-->"+empiric_odl_port_name_for_VMs+" OR "+empiric_odl_port_name_for_routers))
            query = (context.session.query(models_v2.Switches_ports_association).
                 enable_eagerloads(False).filter_by(odl_port_name=empiric_odl_port_name_for_VMs))
            if query.count() == 0:
                #try to see if it is a router port
                query = (context.session.query(models_v2.Switches_ports_association).
                         enable_eagerloads(False).filter_by(odl_port_name=empiric_odl_port_name_for_routers))
        
        '''
        exact_odl_name = "%%%"+ML2_port_id[:11]
        LOG.debug(_("looking for ML2 port called "+exact_odl_name))
        try:
            query = (context.session.query(models_v2.Switches_ports_association).
                enable_eagerloads(False).filter(models_v2.Switches_ports_association.odl_port_name.like(exact_odl_name)))
        except Exception as e:
            LOG.debug(_("exception in query "+str(e)))
            raise e
        '''
        return query.one()
        
    def store_flowmod(self, context,
                    flowrule_id,
                    sw_id,
                    hardTimeout=None,
                    priority=None,
        ...
        args = {
            ...
            'protocol':protocol,
            'actions':actions,
            'flowrule_id':flowrule_id,
            'sw_id':sw_id,
            'incapsulation_vlan':incapsulation_vlan
        }
        LOG.debug(_(args))
        line = models_v2.Flowmod(**args)
        context.session.add(line)
        return id
    
    def delete_flowmod(self, context, id):
        query = (context.session.query(models_v2.Flowmod).
                 enable_eagerloads(False).filter_by(id=id))
        query.delete()

    def get_flowmods_by_flowrule_id(self, context, flowrule_id):
        query = (context.session.query(models_v2.Flowmod).
                 enable_eagerloads(False).filter_by(flowrule_id=flowrule_id))
        LOG.debug(_(query))
        return query
    
    def get_flowmod_by_id(self, context, flowmod_id):
        query = (context.session.query(models_v2.Flowmod).
                 enable_eagerloads(False).filter_by(id=flowmod_id))
        return query.one()
    
    def getFreeVlan(self, context):
        '''
        returns the highest vlan id available (also populates the table if it's empty)
        '''
        #checking if the table is populated or not
        #LOG.debug(_("qui_vlan"))
        query = (context.session.query(models_v2.Vlan).enable_eagerloads(False).filter_by(vlanId=1))
        try:
            query.one()
        except Exception:
            LOG.debug(_("table looks empty, populating..."))
            #fill the table
            for id in range(0,4096):
                ...
                line = models_v2.Vlan(**args)
                context.session.add(line)
        #now the table is ready
        #querying for a free vlan id and setting it to busy
        query = (context.session.query(models_v2.Vlan).enable_eagerloads(False).filter_by(used=0).order_by(models_v2.Vlan.vlanId.desc()))
        LOG.debug(_(query))
        try:
            freeId = query.first()
            LOG.debug(_("id="+str(freeId['vlanId'])))
            return freeId['vlanId']
        except Exception:
            raise n_exc.NoFreeVlan
        
    def setVlanUsageStatus(self, context, vlanId, useCode):
        query = (context.session.query(models_v2.Vlan).enable_eagerloads(False).filter_by(vlanId=vlanId))
        query.one()
        query.update({"used": useCode})
    
    def get_all_flowmods_by_switch_id(self, context, sw_id):
        query = (context.session.query(models_v2.Flowmod).enable_eagerloads(False).filter_by(sw_id=sw_id))
        LOG.debug(_(query))
        return query

    def is_this_port_active(self, context, ML2_port_id):
        query = (context.session.query(models_v2.Port).enable_eagerloads(False).filter_by(id=ML2_port_id).filter_by(status="ACTIVE"))
        query.one()
